      ...
      >
        <div className="w-1/2 h-full flex justify-around items-start">
          {Array.from({ length: 40 }).map((_, i) => (
            <div key={`track1-${i}`} className="w-4 h-6 rounded-sm shadow-md" style={{ backgroundColor: 'var(--color-madeira)' }}></div>
          ))}
        </div>
        <div className="w-1/2 h-full flex justify-around items-start">
          {Array.from({ length: 40 }).map((_, i) => (
            <div key={`track2-${i}`} className="w-4 h-6 rounded-sm shadow-md" style={{ backgroundColor: 'var(--color-madeira)' }}></div>
          ))}
        </div>
      </motion.div>
    </div>
    ...
    <>
      {particles.map((p) => (
        <motion.circle
          ...
          initial={{ opacity: 0.8, scale: 0.5, cx: p.x, cy: p.y }}
          animate={{ opacity: 0, scale: 6, cx: p.x + 250 + Math.random() * 50, cy: p.y - 120 }}
...
const PistonAssembly = () => {
  ...
  return (
    <>
      <motion.line
        x1={x1[0]} x2={x2[0]} y1="160" y2={y2[0]}
        stroke="var(--color-latao)" strokeWidth="6"
        animate={{ x1, x2, y2 }}
        transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
      />
      <motion.rect
        x={cx[0]} y="155" width="20" height="10" fill="var(--color-ferrovia)"
        animate={{ x: cx }}
        transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
      />
      {/* Piston shaft going into the box */}
      <motion.line
        x1={cx[0]} x2="120" y1="160" y2="160"
...
export function AnimatedLocomotive() {
  const [windLines, setWindLines] = useState<{ id: number; y: number; speed: number; delay: number }[]>([]);

  useEffect(() => {
    // Generate static random wind lines
    const lines = Array.from({ length: 15 }).map((_, i) => ({
      id: i,
      y: Math.random() * 100, // Top 100vh
  ...
  return (
    <div className="absolute inset-0 w-full h-full overflow-hidden flex flex-col justify-end pointer-events-none z-0">
      {/* Sun/Moon */}
      <div className="absolute top-10 right-20 w-32 h-32 rounded-full blur-[2px]" style={{ backgroundColor: 'var(--color-amber)', boxShadow: '0 0 80px 20px var(--color-amber)' }}></div>
      ...
      <div className="absolute inset-0 pointer-events-none opacity-30">
        {windLines.map((line) => (
          <motion.div
            ...
            style={{ top: `${line.y}%`, width: `${50 + Math.random() * 100}px` }}
            animate={{ x: ["-10vw", "110vw"] }}
            transition={{ repeat: Infinity, duration: line.speed, delay: line.delay, ease: "linear" }}
          />
        ))}
      </div>

      {/* Clouds */}
      <BackgroundLayer speed={100} className="top-10 z-0 h-64" opacity={0.6} scale={1.2}>
        <Clouds />
      </BackgroundLayer>

      {/* Mountains */}
      <BackgroundLayer speed={60} className="bottom-[120px] z-0 h-64" opacity={0.5} scale={1.1}>
        <Mountains />
      </BackgroundLayer>
      
      {/* Hills */}
      <BackgroundLayer speed={35} className="bottom-[80px] z-10" opacity={0.8} scale={1}>
        <Hills />
      </BackgroundLayer>
      
      {/* Foreground Trees */}
      <BackgroundLayer speed={18} className="bottom-[60px] z-20" opacity={0.9} scale={1.2}>
        <Foreground />
      </BackgroundLayer>
      ...
      {/* Train */}
      <div className="absolute bottom-[58px] md:bottom-[53px] lg:bottom-[45px] left-1/2 -translate-x-1/2 z-30 transform scale-150 md:scale-[1.8] lg:scale-[2] origin-bottom">
        <Locomotive />
      </div>
    </div>
  );
